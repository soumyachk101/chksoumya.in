...
const About = () => {
 ...
 return (
 <section id="about" ref={revealRef} className="reveal-section py-24 relative bg-background overflow-hidden">
 ...
 <Card
 decoration="tack"
 className="bg-surface border-2 border-accent/30 border-wobbly p-6 shadow-hard-lg h-full flex flex-col justify-between text-foreground relative"
 >
 ...
 <span className="text-xs font-mono text-foreground/70 uppercase tracking-widest mt-1.5 block">Class: Fullstack Wizard</span>
 </div>
 <button
 onClick={triggerLevelUp}
 className="px-3 py-1.5 bg-accent hover:bg-accent-hover text-white font-sans font-black text-xs uppercase border-2 border-accent shadow-hard-sm active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all cursor-pointer flex items-center gap-1 shrink-0"
 >
 <span>Level Up</span>
 ...
 <span className="font-mono text-base text-subtle">95 / 100</span>
 </div>
 ...
 <div className="h-3 bg-elevated border border-accent/30 rounded overflow-hidden p-0.5">
 ...
 <div className="h-3 bg-elevated border border-accent/30 rounded overflow-hidden p-0.5">
 <div className="h-full bg-gold rounded" style={{ width: `${speedStat}%` }} />
 </div>
 </div>
 </div>

 {/* Equipped Items list */}
 <div className="bg-surface/80 border border-accent/20 border-dashed rounded p-3 text-xs space-y-1 text-foreground/80">
 <div className="font-bold border-b border-accent/20 pb-1 mb-1 uppercase tracking-widest text-[10px] text-accent">Equipped Inventory</div>
 ...
 <Card decoration="tape" className="bg-surface border-2 border-accent/30 border-wobbly p-6 shadow-hard-lg flex flex-col relative h-[280px] justify-between text-foreground">
 <div className="absolute top-1 right-2 text-[8px] font-mono opacity-70">scribble_scratch.canvas</div>

 <div className="flex justify-between items-center mb-3">
 <h3 className="text-xl font-display font-black text-foreground border-b border-dashed border-pencil">📝 Mini Doodling Canvas</h3>
 <button
 onClick={clearCanvas}
 ...
 <div className="border border-dashed border-accent/40 rounded-lg overflow-hidden bg-base relative flex-1 cursor-crosshair">
 <canvas
 ref={(canvas) => {
 if (canvas) {
 ...
 {/* Coffee Mug widget */}
 <Card className="bg-accent/10 border-2 border-accent/30 border-wobbly p-6 shadow-hard flex flex-col justify-between items-center text-center h-[170px] text-foreground">
 ...
 <div>
 <span className="text-xs font-sans font-bold uppercase block text-foreground/70 mb-1">Caffeine intake</span>
 <span className="text-sm font-sans font-black block mb-3 text-foreground">{coffeeCups} Cups Consumed</span>
 </div>

 <button
 onClick={drinkCoffee}
 className="w-full py-1.5 bg-elevated hover:bg-accent text-foreground hover:text-white font-sans font-black text-xs uppercase border-2 border-accent/30 shadow-hard-sm active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all cursor-pointer"
 >
 Brew & Drink
 </button>
 </Card>
 ...
 <Card className="bg-surface border-2 border-accent/30 border-wobbly p-6 shadow-hard flex flex-col justify-between items-center text-center h-[170px] relative text-foreground">
 <div className="absolute top-1 right-2 text-[8px] font-mono opacity-70">kolkata_compass.db</div>

 {/* Compass Needle Box */}
 <button
 onClick={spinCompass}
 className="w-12 h-12 rounded-full border-2 border-accent/30 bg-base shadow-inner flex items-center justify-center cursor-pointer relative"
 title="Spin compass!"
 ...
 <div className="space-y-1">
 <div className="flex items-center justify-center text-foreground gap-1">
 ...
 <Card variant="post-it" className="rotate-1 hover:rotate-0 p-6 md:p-8 bg-accent/10 border-2 border-accent/30 border-wobbly shadow-hard-lg flex flex-col relative h-[360px] justify-between text-foreground">
 <div className="absolute top-1.5 left-2 text-[10px] text-foreground/70 font-mono">recruiter_hotline.sh</div>

 <div className="border-b border-dashed border-pencil/30 pb-3 mb-3 flex items-center justify-between">
 <span className="text-base font-display font-black text-foreground">💬 Yellow Sticky Messenger</span>
 <div className="flex items-center gap-1.5 text-xs text-foreground/70 font-sans font-bold">
 ...
 {/* Messages Area */}
 <div className="flex-1 bg-elevated border border-accent/20 rounded p-3 text-sm font-sans font-medium overflow-y-auto mb-4 space-y-3 max-h-[170px] select-text">
 {chatLogs.map((msg, idx) => (
 <div
 key={idx}
 className={`flex flex-col max-w-[85%] ${msg.sender === 'user' ? 'ml-auto items-end' : 'mr-auto items-start'}`}
 >
 <span className="text-[9px] uppercase font-bold text-foreground/70 mb-0.5">
 {msg.sender === 'user' ? 'Visitor' : 'Soumya'}
 </span>
 <span
 className={`px-3 py-1.5 border border-accent/30 rounded-lg shadow-hard-sm ${msg.sender === 'user' ? 'bg-accent text-white rounded-tr-none' : 'bg-elevated text-foreground rounded-tl-none'}`}
 >
 {msg.text}
 </span>
 </div>
 ...
 <span className="text-[9px] uppercase font-bold text-foreground/70 mb-0.5">Soumya</span>
 <span className="px-3 py-1.5 border border-accent/30 bg-elevated text-foreground rounded-lg rounded-tl-none italic text-xs flex gap-1 items-center">
 ...
 className="flex-1 py-1.5 px-2.5 bg-elevated hover:bg-accent text-foreground hover:text-white font-sans font-black text-[11px] uppercase border-2 border-accent/30 shadow-hard-sm active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all cursor-pointer flex items-center justify-center gap-1 shrink-0"
 >
 <Send size={10} />
